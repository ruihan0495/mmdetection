...
class CascadePointRendRoIHeadLight(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(CascadePointRendRoIHeadLight, self).__init__(
...
class CascadePointRendRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def init_point_head(self, point_head):
        ...
        for head in point_head:
            self.point_head.append(build_head(head))
    ...
    def _mask_forward_train(self, x, stage, sampling_results, gt_masks,
                            rcnn_train_cfg, img_metas, bbox_feats=None):
        """Run forward function and calculate loss for mask head and point head
        in training."""        
        pos_rois = bbox2roi([res.pos_bboxes for res in sampling_results])
        ...
        mask_targets = self.mask_head[stage].get_targets(sampling_results, gt_masks,
                                                         rcnn_train_cfg)
        pos_labels = torch.cat([res.pos_gt_labels for res in sampling_results])
        ...
        if mask_results['loss_mask'] is not None:
            loss_point = self._mask_point_forward_train(
                x, stage, sampling_results, mask_results['mask_pred'], gt_masks,
                img_metas, rcnn_train_cfg)
            mask_results['loss_mask'].update(loss_point)

        return mask_results

    def _mask_point_forward_train(self, x, stage, sampling_results, mask_pred,
                                  gt_masks, img_metas, rcnn_train_cfg):
        """Run forward function and calculate loss for point head in
        training."""
        pos_labels = torch.cat([res.pos_gt_labels for res in sampling_results])
        rel_roi_points = self.point_head[stage].get_roi_rel_points_train(
            mask_pred, pos_labels, cfg=rcnn_train_cfg)
        rois = bbox2roi([res.pos_bboxes for res in sampling_results])
        ...
        mask_point_pred = self.point_head[stage](fine_grained_point_feats,
                                          coarse_point_feats)
        mask_point_target = self.point_head[stage].get_targets(
            rois, rel_roi_points, sampling_results, gt_masks, rcnn_train_cfg)
        loss_mask_point = self.point_head[stage].loss(mask_point_pred,
                                               mask_point_target, pos_labels)

        return loss_mask_point


    def _mask_point_forward_test(self, x, stage, rois, label_pred, mask_pred,
                                 img_metas):
        """Mask refining process with point head in testing."""
        refined_mask_pred = mask_pred.clone()
        for subdivision_step in range(self.test_cfg.subdivision_steps):
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if self.with_mask and self.with_point:
                mask_results = self._mask_forward_train(
                        x, i, sampling_results, gt_masks, rcnn_train_cfg, img_metas)
                for name, value in mask_results['loss_mask'].items():
                    losses[f's{i}.{name}'] = (
                        value * lw if 'loss' in name else value)
        ...
        if self.with_mask and self.with_point:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                for i in range(self.num_stages):
                    ...
                    aug_mask_rois.append(
                        [r.cpu().numpy() for r in mask_rois])
                ...
                for i in range(num_imgs):
                    ...
                    if det_bboxes[i].shape[0] == 0:
                        ...
                        mask_preds = [mask[i] for mask in aug_masks]
                        mask_rois_i = [mask_roi[i] for mask_roi in mask_rois]
                        x_i = [xx[[i]] for xx in x]
                        mask_rois_i[:, :, 0] = 0  # TODO: remove this hack
                        for j in self.num_stages:
                            mask_pred_i = self._mask_point_forward_test(
                                x_i, i, mask_rois_i[j], det_labels[i], mask_preds[j],
                                [img_metas])
                            aug_point_masks.append(mask_pred_i)
                        merged_masks = merge_aug_masks(
                            aug_point_masks, [[img_metas[i]]] * self.num_stages,
                            rcnn_test_cfg)
            ...
            if det_bboxes.shape[0] == 0:
                ...
                for x, img_meta in zip(features, img_metas):
                    ...
                    for i in range(self.num_stages):
                        mask_results = self._mask_forward(i, x, mask_rois)
                        mask_results['mask_pred'] = self._mask_point_forward_test(
                        x, i, mask_rois, det_labels, mask_results['mask_pred'],
                        img_metas)
                        aug_masks.append(
                            mask_results['mask_pred'].sigmoid().cpu().numpy())
