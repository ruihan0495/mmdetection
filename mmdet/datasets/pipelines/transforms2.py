...
class Resize(object):
    ...
    def _resize_bboxes(self, results):
        ...
        for key in results.get('bbox_fields', []):
            if results[key].shape[-1] > 4:
                scale_factor = np.concatenate((results['scale_factor'], [1,1]), axis=None)
            ...
            #print('bboxes after resize:', bboxes)
            results[key] = bboxes
...
class Albu(object):
    ...
    def __call__(self, results):
        if ('paste_image' in results) and ('paste_masks' in results) and ('paste_bboxes' in results):
            ...
            del results['paste_image']
            del results['paste_masks']
            del results['paste_bboxes']
